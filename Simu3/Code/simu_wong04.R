...
simu <- function(diffmu, weakSignalRatio, ntry){
  ...
  ## MOGC ##
  res <- MOGC(Y, U1, U2, K=3, center=FALSE, 
    burnInIter=1000, keepIter=3000, maxIter=10000, 
  ...
  ## MOGC_DP
  K <- 10
  res <- MOGC_DPT(Y, U1, U2, K=K, center=FALSE, 
    burnInIter=1000, keepIter=3000, maxIter=10000, 
    print_int=1000, debug=FALSE, init_mu=NULL, 
    init_z=res_spkm[[1]]$Cs, fix_z=FALSE, 
    fix_mu=FALSE, adj_ls=TRUE, 
  ...
  ARI_mog_dp <- adjustedRandIndex(res$label_map,Sdata$label)
  ...
  save(ARI_sog, AUC_sog, ARI_sog_dp, AUC_sog_dp, 
    ARI_mog, AUC_mog, ARI_mog_dp, AUC_mog_dp,
