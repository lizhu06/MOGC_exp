...
wd1 <- paste0(server, "Exp2/Simu3/Output/")
wd2 <- paste0(server, "Exp2/Simu3/Output_v2/")
res_wd <- paste0(server, "Exp2/Simu3/Results/")
#setwd(wd)
...
for(diffmu_ind in 1:length(diffmu_uni)){
  ...
  for(weak_ind in 1:length(weak_uni)){
    ...
    for(ntry_ind in 1:num_try){
      ...
      load(paste0(wd1, "Simu3_diffmu_",diffmu, 
      ...
      load(paste0(wd2, "Simu3_diffmu_",diffmu, 
        "_weakSignalRatio_", weakSignalRatio, "_ntry_", ntry, "_SPKM_init.RData"))
      AUC_m[2, diffmu_ind, weak_ind, ntry_ind] <- AUC_sog
      AUC_m[3, diffmu_ind, weak_ind, ntry_ind] <- AUC_sog_dp
      AUC_m[7, diffmu_ind, weak_ind, ntry_ind] <- AUC_mog
      AUC_m[8, diffmu_ind, weak_ind, ntry_ind] <- AUC_mog_dp

      ARI_m[2, diffmu_ind, weak_ind, ntry_ind] <- ARI_sog
      ARI_m[3, diffmu_ind, weak_ind, ntry_ind] <- ARI_sog_dp
      ARI_m[7, diffmu_ind, weak_ind, ntry_ind] <- ARI_mog
      ARI_m[8, diffmu_ind, weak_ind, ntry_ind] <- ARI_mog_dp
      ...
      AUC_long[index+seq(1,num_methods), 4] <- c(AUC_spkm,
        AUC_sog, AUC_sog_dp, AUC_iskm_vec, AUC_mog, AUC_mog_dp)
      ...
      ARI_long[index+seq(1,num_methods), 4] <- c(ARI_spkm,
        ARI_sog, ARI_sog_dp, ARI_iskm_vec, ARI_mog, ARI_mog_dp)
...
ARI_plot <- ggplot(ARI_long, aes(x=method, y=ARI, group=method)) + 
  ...
  scale_x_discrete(name="", labels=c("SPKM"="SPKM", 
    ...
    "MOGC"="MOGC",
    "MOGCDP"=parse(text=TeX("MOGC$_{dp}$"))))+
  labs(title="Multi-layer overlapping")

pdf(file=paste0(res_wd, "../Results/Compare_ARI_SPKM_init_trimed_v2.pdf"), 
  width=4.5, height=4)
...
grid.arrange(ARI_plot, AUC_plot, nrow = 1,
  top="Multi-layer overlapping groups")
dev.off()

simu3_plot <- list(ARI_plot, AUC_plot)
save(simu3_plot, file=paste0(res_wd, "ARI_AUC_plot_list_v2.RData"))
