...
wd <- paste0(server, "Exp2/Simu3/Output/")
setwd(wd)
...
for(diffmu_ind in 1:length(diffmu_uni)){
  ...
  for(weak_ind in 1:length(weak_uni)){
    ...
    for(ntry_ind in 1:num_try){
      ...
      load(paste0("Simu3_diffmu_",diffmu, 
...
pdf(file="../Results/Compare_ARI_SPKM_init_trimed.pdf", 
  width=4.5, height=4)
...
pdf(file="../Results/Compare_AUC_SPKM_init_trimed.pdf", 
  width=4.5, height=4)
par(mfrow=c(2,4), mar=c(5,5,2,1), par(cex.axis=0.8))
plot(AUC_plot)
dev.off()
...
pdf(file="../Results/Combine_ARI_AUC_trimed.pdf", width=7, height=3.5)
grid.arrange(ARI_plot, AUC_plot, nrow = 1,
  top="Multi-layer overlapping groups")
dev.off()

simu3_plot <- list(ARI_plot, AUC_plot)
save(simu3_plot, file="../Results/ARI_AUC_plot_list.RData")
