...
load(paste0(res_wd, "res_spkm_tune.RData"))
...
# score results
logrank_p_table <- ARI_table <- matrix(NA, 7, 1)
...
(ARI_table[3,1] <- adjustedRandIndex(res_iskm_alpha001$Cs, 
	pam50_2)) 
for(i in 1:4){
	ARI_table[i+3,1] <- tryCatch(adjustedRandIndex(all_res[[i]]$label_map, 
		pam50_2), error=function(e) NA)
}
...
## survival test
library(survival)
load("/net/wong04/home/liz86/MOG_clustering/RawData/Complete_METABRIC_Clinical_Survival_Data__DSS.rbin")
surv_obj <- Complete_METABRIC_Clinical_Survival_Data__DSS[match(rownames(d_nodup), 
	rownames(Complete_METABRIC_Clinical_Survival_Data__DSS)), ]
dim(surv_obj) #1975    2
all(rownames(surv_obj)==rownames(d_nodup))

get_logrank_p <- function(z){
	fit <- survdiff(surv_obj ~ z)
	p <- 1 - pchisq(fit$chisq, length(unique(z))-1)
	return(p)
}

(logrank_p_table[1,1] <- get_logrank_p(res_spkm[[1]]$Cs))
(logrank_p_table[2,1] <- get_logrank_p(res_iskm_alpha05$Cs))
(logrank_p_table[3,1] <- get_logrank_p(res_iskm_alpha001$Cs)) 
for(i in 1:4){
	logrank_p_table[i+3,1] <- tryCatch(get_logrank_p(all_res[[i]]$label_map),
	 error=function(e) NA)
}
write.csv(logrank_p_table, file=paste0(res_wd, "logrank_p_table.csv"))
table(res_spkm[[1]]$Cs, res_sogc$label_map)
table(res_spkm[[1]]$Cs, res_mogc$label_map)
