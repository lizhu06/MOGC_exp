...
simu <- function(diffmu, weakSignalRatio, ntry){
  ...
  res_wd <- "/net/wong04/home/liz86/MOG_clustering/Exp2/Simu2/"

  #wd <- "/mnt/glusterfs/liz86/MOGC/Exp2/SOG_overlap_setting/"
  setwd(res_wd)
...
#  AUC_spkm <- auc(roc(factor(Sdata$geneLabel), res_spkm[[1]]$ws))  
  ...
  res_sogc_dp <- SOGC_DPT(Y, U1, K=10, center=FALSE, 
    ...
    BernoulliWeighted=TRUE, MH_ind=1,
  ...
  save(ARI_sogc, AUC_sogc, ARI_sogc_dp, AUC_sogc_dp, 
    file=paste0(savewd, "Simu2_diffmu_",
    diffmu, "_weakSignalRatio_", weakSignalRatio, "_ntry_", ntry, "_SPKM_init.RData"))
}
