...
wd <- paste0(server, "Exp2/Simu2/Output/")
setwd(wd)
...
for(diffmu_ind in 1:length(diffmu_uni)){
  ...
  for(weak_ind in 1:length(weak_uni)){
    ...
    for(ntry_ind in 1:num_try){
      ...
      load(paste0("Simu2_diffmu_",diffmu, 
      ...
      #AUC_m[6, diffmu_ind, weak_ind, ntry_ind] <- AUC_mog
      #AUC_m[7, diffmu_ind, weak_ind, ntry_ind] <- AUC_mog_dp
      ...
      #ARI_m[6, diffmu_ind, weak_ind, ntry_ind] <- ARI_mog
      #ARI_m[7, diffmu_ind, weak_ind, ntry_ind] <- ARI_mog_dp
...
ARI_plot <- ggplot(ARI_long, aes(x=method, y=ARI, group=method)) + 
  ...
  labs(title="Single-layer overlapping")
...
grid.arrange(ARI_plot, AUC_plot, nrow = 1,
  top="Single-layer overlapping groups")
dev.off()

simu2_plot <- list(ARI_plot, AUC_plot)
save(simu2_plot, file="../Results/ARI_AUC_plot_list.RData")
