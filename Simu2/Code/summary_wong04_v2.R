...
wd1 <- paste0(server, "Exp2/Simu2/Output/")
wd2 <- paste0(server, "Exp2/Simu2/Output_v2/")
res_wd <- paste0(server, "Exp2/Simu2/Results/")
#setwd(res_wd)
...
for(diffmu_ind in 1:length(diffmu_uni)){
  ...
  for(weak_ind in 1:length(weak_uni)){
    ...
    for(ntry_ind in 1:num_try){
      ...
      load(paste0(wd1, "Simu2_diffmu_",diffmu, 
      ...
      load(paste0(wd2, "Simu2_diffmu_",diffmu, 
        "_weakSignalRatio_", weakSignalRatio, "_ntry_", ntry, "_SPKM_init.RData"))
...
save(simu2_plot, file=paste0(res_wd, "ARI_AUC_plot_list_v2.RData"))
